...
WP_TMP=/tmp/wp-lang
v=3.5.2
d=/srv/www/postholic.com/public_html/wp-content

# Download original translated packages
echo " - Downloading WordPress packages..."
mkdir -p $WP_TMP/
wget -qN http://cn.wordpress.org/wordpress-$v-zh_CN.zip -P $WP_TMP/ -a $WP_TMP/wget.log -d
wget -qN http://ja.wordpress.org/wordpress-$v-ja.zip -P $WP_TMP/ -a $WP_TMP/wget.log -d

# Unpack
echo " - Unpacking zipball..."
unzip -qqo $WP_TMP/wordpress-$v-zh_CN.zip -d $WP_TMP/cn/
unzip -qqo $WP_TMP/wordpress-$v-ja.zip -d $WP_TMP/ja/
...
cp -R $WP_TMP/cn/wordpress/wp-content/languages/ $d/
cp -R $WP_TMP/ja/wordpress/wp-content/languages/ $d/
